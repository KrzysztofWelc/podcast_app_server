...
class TestPodcastsPackage(BaseTestCase):
    ...
    def test_comment_get(self):
        with self.client:
            response = self.client.get("/api/comments/{}/2".format(self.podcast.id))
    ...
    def test_comment_get_end_of_list(self):
        with self.client:
            response = self.client.get("/api/comments/{}/3".format(self.podcast.id))
    ...
    def test_comment_delete(self):
        with self.client:
            ...
            response = self.client.delete(
                "/api/comments",
                data=json.dumps(dict(comment_id=comment.id)),
                content_type="application/json",
                headers=dict(authToken="Bearer " + self.user.generate_auth_token()),
            )
    ...
    def test_comment_update(self):
        with self.client:
            ...
            response = self.client.put(
                "/api/comments",
                data=json.dumps(dict(text=updated_text, comment_id=comment.id)),
                content_type="application/json",
                headers=dict(authToken="Bearer " + self.user.generate_auth_token()),
            )
    ...
    def test_comment_answer_not_permitted(self):
        ...
        with self.client:
            response = self.client.post(
                ...
                data=json.dumps({"text": answer_text}),
                content_type="application/json",
                headers=dict(authToken="Bearer " + wrong_token),
            )
    ...
    def test_get_paginated_answers(self):
        ...
        for i in range(50):
            a = AnswerComment(
                text=faker.sentence(nb_words=20), comment=comment, user_id=self.user.id
            )
            db.session.add(a)
